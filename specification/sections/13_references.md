# References

[1] OWASP GenAI Security Project. (2026). OWASP Top 10 for LLM Applications 2026. Retrieved from https://genai.owasp.org/resource/owasp-genai-llm-top-10-2026/

[2] OWASP. (2026). OWASP Top 10 for Agentic AI Applications 2026. Retrieved from https://genai.owasp.org/

[3] MITRE Corporation. (2026). MITRE ATLAS: Adversarial Threat Landscape for Artificial-Intelligence Systems, data version 2026.07. Retrieved from https://github.com/mitre-atlas/atlas-data/releases/tag/v2026.07

[4] Cloud Security Alliance. (2025). MAESTRO Framework: Multi-layered Agentic AI Threat Modeling. Retrieved from https://cloudsecurityalliance.org/blog/2025/02/06/agentic-ai-threat-modeling-framework-maestro

[5] Cloud Security Alliance. (2026). AI Controls Matrix 1.1: 247 control objectives across 18 domains. Retrieved from https://cloudsecurityalliance.org/artifacts/ai-controls-matrix-v1-1

[6] NIST. (2023). NIST AI Risk Management Framework (AI RMF 1.0). NIST AI 100-1. Retrieved from https://www.nist.gov/itl/ai-risk-management-framework

[7] NIST. (2025). NIST IR 8596 Initial Preliminary Draft: Cybersecurity Framework Profile for Artificial Intelligence (Cyber AI Profile). Retrieved from https://csrc.nist.gov/pubs/ir/8596/iprd

[8] ISO/IEC. (2023). ISO/IEC 42001:2023 Information technology — Artificial intelligence — Management system. International Organization for Standardization.

[9] ISO/IEC. (2025). ISO/IEC 42005:2025 Information technology — Artificial intelligence — AI system impact assessment. International Organization for Standardization.

[10] European Union. (2024, amended 2026). Regulation (EU) 2024/1689 on Artificial Intelligence, as amended by Regulation (EU) 2026/1744. Official Journal of the European Union.

[11] OWASP. (2026). AI Security Verification Standard (AISVS) 1.0. Retrieved from https://github.com/OWASP/AISVS

[12] AIDEFEND Framework. (2026). AI Defense Framework for Security Controls. Retrieved from https://aidefend.net/

[13] SPIFFE. (2025). Secure Production Identity Framework For Everyone. Retrieved from https://spiffe.io/

[14] Model Context Protocol. (2026). Model Context Protocol Specification, revision 2026-07-28. Retrieved from https://modelcontextprotocol.io/specification/2026-07-28

[15] Zhang, Y., Nazaraliyev, R., Dutta, S. B., Marquez, A., Barker, K., & Abu-Ghazaleh, N. (2025). NVBleed: Covert and Side-Channel Attacks on NVIDIA Multi-GPU Interconnect. arXiv:2503.17847.

[16] OWASP. (2026). AIVSS Scoring System for OWASP Agentic AI Core Security Risks v0.8. Retrieved from https://aivss.owasp.org/

[17] Artificial Intelligence Underwriting Company. (2026). AIUC-1: The Certification Standard for AI Agents; AIUC company and insurance overview. Retrieved from https://www.aiuc-1.com/ and https://aiuc.com/

[18] OWASP. (2025). Agentic AI - Threats and Mitigations 1.1. Retrieved from https://genai.owasp.org/resource/agentic-ai-threats-and-mitigations/
